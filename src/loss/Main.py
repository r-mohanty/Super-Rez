...
class Adversarial(nn.Module):
    def __init__(self, args, G, fidelity_weight, regularization_weight, checkpoint=None):
        super(Adversarial, self).__init__()
        self.device = torch.device('cpu' if args.cpu else 'cuda')
        self.dis = Discriminator(BasisSize=3, StageWidths=[256, 256, 512, 512, 512, 1024], BlocksPerStage=[1, 1, 2, 2, 2, 1]).to(self.device)
        
        if args.resume == -1:
            load_from_dis = torch.load(
                os.path.join('..', 'experiment', 'cs143_x4_gen', 'model', 'dis_latest.pt'), map_location='cpu')
            self.dis.load_state_dict(load_from_dis, strict=False)
        
        self.optimizer = utility.make_optimizer(args, self.dis, is_dis=True)
        
        if args.resume == -1:
            load_from_opt = torch.load(
                os.path.join('..', 'experiment', 'cs143_x4_gen', 'model', 'dis_opt_latest.pt'), map_location='cpu')
            self.optimizer.load_state_dict(load_from_opt)
